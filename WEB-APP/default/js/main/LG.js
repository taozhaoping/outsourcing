...
*/
    ...
    //快速设置表单底部默认的按钮:保存、取消
    LG.setFormDefaultBtn = function (cancleCallback, savedCallback,listCallback)
    {
        //表单底部按钮
        var buttons = [];
        if (cancleCallback)
        ...
        if (listCallback)
        {
            buttons.push({ text: '通信记录', onclick: listCallback });
        }
  ...
  //填充表单数据
    ...
    //带验证、带loading的提交
    LG.submitForm = function (mainform,className, success, error)
    {
        if (!mainform)
            mainform = $("form:first");
        if (mainform.valid())
        {
            mainform.ajaxSubmit({
                ...
                beforeSubmit: function (formData, jqForm, options)
                {
                    ...
                    LG.nameChangeOgnl(formData,className);
                },
    ...
    //表单提交前的数据name转换。使ognl能够正常获取参数,select 下拉列表有问题，删除所有name为undefined的对象
    LG.nameChangeOgnl = function(form , className)
    {
		for (i in form) {
			if (form[i].name =='undefined')
			{
				...
				form[i].name = className + '.' + form[i].name;
			}
		}
    };
    ...
    {
        if(top.tab.isTabItemExist(parentTabid))
        {
        	//top.tab.selectTabItem(parentTabid);
        	top.tab.reload(parentTabid);
        }
